...
void main() {
    if (GetTimeMinute() == 0) {
        ...
        while(GetIsObjectValid(oPlayer)) {
            ...
            SendMessageToPC(oPlayer, GetToken(103) + "Die Auﬂentemperatur betr‰gt " + IntToString(iTemperatur) + "∞C</c>");
            ...
            if (iWindStrength == 1) {
                ...
                SendMessageToPC(oPlayer, GetToken(103) + "Es weht ein schwacher " + sWindDirection + " .</c>");
            } else if (iWindStrength == 3) {
                SendMessageToPC(oPlayer, GetToken(103) + "Es weht ein mittelstarker " + sWindDirection + ".</c>");
            } else if (iWindStrength == 4) {
                SendMessageToPC(oPlayer, GetToken(103) + "Es weht ein starker " + sWindDirection + "</c>");
            } else if (iWindStrength == 5) {
                SendMessageToPC(oPlayer, GetToken(103) + "Es weht ein sehr starker " + sWindDirection + ".</c>");
            }
